...
form.addEventListener('submit', (event) => {
	...
	// let nameInputLowerCase = nameInput.toLowerCase();
	...
	if(classDurationInput){
		...
		let seconds = classDurationInput * 60; //converting into seconds
...
formdata.append("user_id", nameInput);
...
fetch("https://staging-preprod.vishwamcorp.com/v2/single_gesture_ios", requestOptions)
	...
	//  console.log(v);
		...
		if(status == 404 || status == 424 || status == 500 || status == 406 || status == 409){
...
////////////////////////
		...
		if( x == count)
		{
			...
			let presentPercentage = nameInput + "'s present percentage is:" + " " + pPercentage;
